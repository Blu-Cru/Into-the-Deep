...
import org.firstinspires.ftc.teamcode.blucru.common.pathbase.specimen.SpecimenCycleDepositSplinePath;
...
public class FiveSpecimenSplineConfig extends AutoConfig {
    int scoreCount;
    int spitCount = 0;
    Path[] collectPaths;
    ...
    public FiveSpecimenSplineConfig() {
        ...
        sm = new StateMachineBuilder()
                ...
                .transition(() -> currentPath.isDone() && spitCount >= 2, State.INTAKING_CYCLE, () -> {
                    currentPath = new SpecimenCycleIntakeSplinePath().build().start();
                })
                ...
                .transition(() -> currentPath.isDone() || (Robot.getInstance().intakeSwitch.pressed() && Robot.getInstance().pivot.getAngle() < 0.35),
                        State.DEPOSIT_CYCLE, () -> {
                            currentPath = new SpecimenCycleDepositSplinePath(scoreCount).build().start();
                        })
    ...
    public void start() {
        scoreCount = 0;
        currentPath = new SpecimenPreloadDepositSplinePath().build().start();
